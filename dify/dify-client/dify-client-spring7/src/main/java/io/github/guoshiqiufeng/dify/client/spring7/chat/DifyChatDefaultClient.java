 ...
 */
package io.github.guoshiqiufeng.dify.client.spring7.chat;
...
import io.github.guoshiqiufeng.dify.client.spring7.utils.MultipartInputStreamFileResource;
    ...
    public List<String> messagesSuggested(String messageId, String apiKey, String userId) {
        RestClient.RequestHeadersSpec<?> uri = this.restClient.get()
                .uri(ChatUriConstant.V1_MESSAGES_URI + "/{messageId}/suggested?user={user}",
                        messageId,
                        userId);
        DifyResult<List<String>> body = uri
                .header(HttpHeaders.AUTHORIZATION, "Bearer " + apiKey)
                .retrieve()
                .onStatus(responseErrorHandler)
