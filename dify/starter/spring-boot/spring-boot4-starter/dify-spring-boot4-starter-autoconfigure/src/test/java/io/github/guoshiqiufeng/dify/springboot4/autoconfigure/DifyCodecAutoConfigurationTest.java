...
public class DifyCodecAutoConfigurationTest {
    ...
    void autoconfigurationActivatesWithJsonMapperClassOnClasspath() {
        this.contextRunner
                .run((context) -> {
                    ...
                    assertThat(context.getBean(JsonMapper.class)).isInstanceOf(Jackson3JsonMapper.class);
                });
    }
    ...
    void autoconfigurationIsDisabledWhenJacksonJsonMapperClassIsNotPresent() {
        this.contextRunner
                .withClassLoader(new FilteredClassLoader(Jackson3JsonMapper.class))
                .run((context) -> {
                    assertThat(context).doesNotHaveBean(JsonMapper.class);
                });
    }
    ...
    void defaultJsonMapperIsJacksonJsonMapper() {
        this.contextRunner
                .run((context) -> {
                    ...
                    assertThat(jsonMapper).isInstanceOf(Jackson3JsonMapper.class);
                    assertThat(jsonMapper).isSameAs(Jackson3JsonMapper.getInstance());
                });
    }
