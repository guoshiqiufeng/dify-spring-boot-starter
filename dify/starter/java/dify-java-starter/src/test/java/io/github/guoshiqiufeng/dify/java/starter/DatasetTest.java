...
/**
 * Dataset API integration tests using builder pattern
 *
 * @author yanghq
 * @version 1.0
...
public class DatasetTest {
    ...
    private static DifyFile testFile;
    ...
    public static void setup() throws IOException {
        ...
        // Initialize DifyServer to get dataset API key
        ...
        testFile = createTestFile("test.txt", "text/plain");
    }
    ...
    public void testDocumentIndexingStatus() throws InterruptedException {
        ...
        assertNotNull(response);
        int attempts = 0;
        while (!response.getData().get(0).getIndexingStatus().equals("completed") && attempts < 30) {
            attempts++;
            Thread.sleep(500);
    ...
    public void testSegmentOperations() throws InterruptedException {
        ...
        SegmentData segmentData = createResponse.getData().get(0);
    ...
    public void testSegmentChildOperations() throws InterruptedException {
        ...
        assertNotNull(response);
        int attempts = 0;
        while (!statusResponse.getData().get(0).getIndexingStatus().equals("completed") && attempts < 30) {
            attempts++;
            Thread.sleep(500);
        ...
        assertNotNull(createResponse);
        assertNotNull(createResponse.getData());
        String segmentId = createResponse.getData().get(0).getId();
